    ...
    while is_queue_empty() == False:
        ...
        for n in range(0, len(node[5])):
            ...
            if graph[nbr[0]][1] == False:
                _add_to_queue(nbr[0], node_id,
                              graph[node_id][6][nbr[0]] * a + _heuristic(graph, nbr[0]) * b)
...
if __name__ == "__main__":
    ...
    (expansion_count, cost, path) = _graph_search(
        graph, 1, 5, mp.add_to_queue_DFS, mp.is_queue_empty_DFS, mp.pop_front_DFS, _heuristic_zero)
    ...
    graph = _get_test_graph_admissible()
    (expansion_count, cost, path) = _graph_search(
        graph, 1, 5, mp.add_to_queue_BFS, mp.is_queue_empty_BFS, mp.pop_front_BFS, _heuristic_zero)
    ...
    graph = _get_test_graph_admissible()
    (expansion_count, cost, path) = _graph_search(
        graph, 1, 5, mp.add_to_queue_UC, mp.is_queue_empty_UC, mp.pop_front_UC, _heuristic_zero)
    ...
    (expansion_count, cost,
     path) = _graph_search(graph, 1, 5, mp.add_to_queue_ASTAR, mp.is_queue_empty_ASTAR,
                           mp.pop_front_ASTAR, _heuristic_function)
    ...
    print "A valid solution: " + str(
        mp.n_queens(n, mp.get_random_state, mp.compute_attacking_pairs, mp.hill_desending_n_queens))
