...
def _background_logic():
    ...
    while not PG_GAME_EXIT:
        ...
        if (x != oldx or y != oldy) and (not PG_LASER_ON):
            ...
            # Calling the shots...
            (ex, ey, shoot) = mp.kalman2d_shoot(
                ux, uy, x + oxn[0], y + oyn[0], first_iteration)
            ...
            if shoot == True:
                PG_LASER_ON = True
                print "Shot fired!"
                print "Actual target location is (" + str(
                    TARGET_LOCATION[0]) + "," + str(TARGET_LOCATION[1])
                print "Estimated target location is (" + \
                    str(ex) + "," + str(ey)
        ...
        if PG_UI_UPDATED == True:
            ...
            if PG_LASER_ON:
                ...
                PG_SCREEN.blit(PG_LASER_IMAGE, (rx + ex +
                                                PG_LASER_IMAGE_WIDTH/2 + 2, 0))
